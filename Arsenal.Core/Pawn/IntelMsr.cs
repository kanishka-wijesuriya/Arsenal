    ...
    public sealed class IntelMsr : IDisposable
    {
        ...
        public bool Initialize(Assembly assembly)
        {
            ...
            using var stream = assembly.GetManifestResourceStream(name)
                ?? assembly.GetManifestResourceStream("Arsenal.Core.IntelMSR.bin")
                ?? assembly.GetManifestResourceStream("Arsenal.IntelMSR.bin")
                ?? (Array.Find(assembly.GetManifestResourceNames(), n => n.EndsWith("IntelMSR.bin", StringComparison.OrdinalIgnoreCase)) is string foundName
