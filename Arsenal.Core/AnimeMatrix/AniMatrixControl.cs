...
namespace Arsenal.AnimeMatrix
{
    ...
    {
        public static event Action<int>? OnMatrixRunningVisualise;
        public static event Action<string>? OnMatrixPictureVisualise;
        ...
        {
            // Same guard every other method in this class opens with. Without it a
            // Slash-only machine - deviceSlash present, deviceMatrix null - crashed here
            // rather than doing nothing, which is what an absent matrix should mean.
            if (deviceMatrix is null) return;
        ...
        public void OpenMatrixPicture()
        {
            ...
            if (fileName is not null)
            {
                ...
                OnMatrixRunningVisualise?.Invoke((int)MatrixMode.Picture);
            }
        }
        ...
        public void SetMatrixPicture(string fileName)
        {
            ...
            try
            {
                using (var fs = new FileStream(fileName, FileMode.Open))
                {
                    ...
                    fs.Close();
                    OnMatrixPictureVisualise?.Invoke(fileName);
                }
            }
            catch
            {
        ...
        /// <summary>
        /// How long to hold one frame of an animated image, in milliseconds.
        /// </summary>
        /// <remarks>
        /// 0x5100 is the GIF frame-delay property. A GIF that omits it, an image that is
        /// not a GIF at all, or a truncated value all used to throw here - from a user
        /// picking their own file, which is exactly where an unexpected image comes from.
        /// The configured speed is the floor anyway, so an unreadable delay falls back to
        /// it instead of failing.
        /// </remarks>
        public static int PictureFrameDelay(Image image)
        {
            int configured = AppConfig.Get("matrix_speed", 50);
            try
            {
                byte[]? value = image.GetPropertyItem(0x5100)?.Value;
                if (value is null || value.Length < 4) return configured;
                return Math.Max(configured, BitConverter.ToInt32(value) * 10);
            }
            catch (Exception)
            {
                return configured;
