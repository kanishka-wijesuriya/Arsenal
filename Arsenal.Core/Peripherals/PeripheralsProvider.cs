...
using Arsenal.Peripherals.Mouse.Models;
using Arsenal.USB;
using HidSharp;
...
namespace Arsenal.Peripherals
{
    public class PeripheralsProvider
    {
        ...
        public static List<AsusKeyboard> SnapshotKeyboards()
        {
            lock (_LOCK)
            {
                return new List<AsusKeyboard>(ConnectedKeyboards);
            }
        }
        ...
        public static event Action? OnPeripheralsChanged;
        ...
        private static void DeviceTimer_Elapsed(object? sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            // This runs on a timer thread in response to a device arriving, which can
            // happen before the dispatcher is constructed or after shutdown has cleared
            // it. An exception here has nothing above it to catch it.
