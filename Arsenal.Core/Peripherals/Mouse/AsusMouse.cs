using Arsenal.AnimeMatrix.Communication;
using Arsenal.AnimeMatrix.Communication.Platform;
using Arsenal.USB;
using System.Runtime.CompilerServices;
using System.Text;

namespace Arsenal.Peripherals.Mouse
{
    ...
    public abstract class AsusMouse : Device, IPeripheral
    {
        ...
        //GMP1 = Arsenal Mouse Profile Version 1 :D
        ...
        //This function should automatically disconnect the device in Arsenal if the device is no longer there or the pipe is broken.
        ...
        public void SetDPIProfileCount(int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    // GetUpdateDPIPacket returns null for a profile this model does not
                    // support, the same as it does at the single-profile call site above,
                    // which checks. Skipping one profile is right; taking the resync down
                    // and leaving the remaining profiles unwritten is not.
                    byte[]? packet = GetUpdateDPIPacket(DpiSettings[i], i + 1);
                    if (packet == null)
                    {
                        Logger.WriteLine(GetDisplayName() + ": DPI setting for profile " + (i + 1) + " does not exist or is invalid.");
                        continue;
                    }
                    WriteForResponse(packet);
                }
