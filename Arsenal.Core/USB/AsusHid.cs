...
public static class AsusHid
{
    ...
    /// <summary>
    /// Every ASUS HID device that answers to <paramref name="reportId"/>.
    /// </summary>
    /// <remarks>
    /// Declared nullable, which it never was: this is an iterator, so it returns an
    /// enumerator object even on the failure path - the <c>yield break</c> in the catch
    /// produces an empty sequence, not null. The annotation made the compiler warn at
    /// every <c>foreach</c> over it and made two callers write null checks that could
    /// not fire, while the real failure mode (an empty result) went unremarked.
    /// </remarks>
