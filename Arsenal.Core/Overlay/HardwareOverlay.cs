    ...
    public class HardwareOverlay : OSDNativeForm
    {
        ...
        protected override void WndProc(ref Message m)
        {
            if (m.Msg == WM_NCDESTROY)
            {
                base.WndProc(ref m);
                if (_active)
                    Program.Bridge?.RunOnUi(() => { RestorePosition(); base.Show(); });
                return;
            }
        ...
        // since _currentFps still reflects the old foreground here). UI thread, so no
        // lock needed.
        ...
        {
            if (!_active) return;
            Program.Bridge?.RunOnUi(() => { if (_active) RestorePosition(); });
        }
