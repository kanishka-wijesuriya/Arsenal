...
public static class HardwareControl
{
    ...
    private static readonly object _batteryQueryLock = new();
    private static Task<BATTERY_STATUS?>? _batteryQueryTask;

    /// <summary>
    /// DeviceIoControl is normally immediate, but a broken battery driver can leave a
    /// query blocked. Reuse that one in-flight operation after the UI timeout instead of
    /// starting another blocked task every five seconds and growing the thread pool.
    /// </summary>
    private static BATTERY_STATUS? QueryBatteryStatusWithTimeout()
    {
        Task<BATTERY_STATUS?> query;
        lock (_batteryQueryLock)
        {
            _batteryQueryTask ??= Task.Run(QueryBatteryStatus);
            query = _batteryQueryTask;
        }

        try
        {
            if (!query.Wait(1000)) return null;
        ...
        finally
        {
            if (query.IsCompleted)
            {
                lock (_batteryQueryLock)
                {
                    if (ReferenceEquals(_batteryQueryTask, query)) _batteryQueryTask = null;
                }
            }
        }
    }
    ...
    public static void ReadBatteryState()
    {
        ...
        try
        {
            var directStatus = QueryBatteryStatusWithTimeout();
    ...
    public static void InitCPUPowerAsync()
    {
        ...
        Task.Run(() =>
        {
            ...
            {
                EnergyChannel? apuChannel = null;
                EnergyChannel? cpuChannel = null;
                try
                {
                    apuChannel = new EnergyChannel("Apu Power");
                    cpuChannel = new EnergyChannel("CPU Power");
                    _apuChannel = apuChannel;
                    _cpuChannel = cpuChannel;
            ...
            {
                List<PerformanceCounter>? cores = null;
                try
                {
                ...
                finally { DisposeCounters(cores); }
            }

            // Try cached instance name first; this skips the PerformanceCounterCategory
            ...
            if (!string.IsNullOrEmpty(cached))
            {
                ...
                try
                {
                    counter = new PerformanceCounter("Energy Meter", "Power", cached, true);
                    counter.NextValue();
            ...
            try
            {
                ...
                foreach (var name in _powerCounterInstances)
                {
                    if (instances.Contains(name, StringComparer.OrdinalIgnoreCase))
                    {
                        ...
                        try
                        {
                            counter = new PerformanceCounter("Energy Meter", "Power", name, true);
                            counter.NextValue();
                            _cpuPowerCounter = counter;
                            counter = null;
    ...
    private sealed class EnergyChannel : IDisposable
    {
    ...
    private static float? GetCoresPower()
    {
        ...
        {
            DisposeCounters(counters);
            _coreCounters = null;
            return null;
        }
    }

    private static void DisposeCounters(IEnumerable<PerformanceCounter>? counters)
    {
    ...
    // Direct Win32 API (the same source Task Manager uses); avoids PerformanceCounter
    ...
    private static float? GetIntelMsrPower()
    {
        ...
        PawnIO.IntelMsr? candidate = null;
        try
        {
            if (_intelMsr == null)
            {
                candidate = new PawnIO.IntelMsr();
                if (!candidate.Initialize(typeof(HardwareControl).Assembly))
                {
                    candidate.Dispose();
                    candidate = null;
    ...
    public static void ReadSensors(bool log = false)
    {
        ...
        int cpuFanRaw = Program.acpi.GetFan(AsusFan.CPU);
        int gpuFanRaw = Program.acpi.GetFan(AsusFan.GPU);
        int midFanRaw = Program.acpi.GetFan(AsusFan.Mid);

        cpuFan = FanSensorControl.FormatFan(AsusFan.CPU, cpuFanRaw);
        gpuFan = FanSensorControl.FormatFan(AsusFan.GPU, gpuFanRaw);
        midFan = FanSensorControl.FormatFan(AsusFan.Mid, midFanRaw);

        // Dashboard polling uses the full sensor pass rather than the overlay's
        // lightweight pass. Keep both formatted legacy values and numeric RPMs
        // in sync from the same ACPI samples so the UI cannot be stuck at zero.
        if (readFans)
        {
            cpuFanRPM = cpuFanRaw >= 0 ? cpuFanRaw * 100 : null;
            gpuFanRPM = gpuFanRaw >= 0 ? gpuFanRaw * 100 : null;
        }
        else
        {
            cpuFanRPM = null;
    ...
    public static void Dispose()
    {
        ...
        if (_amdApuControl is not null && !ReferenceEquals(_amdApuControl, GpuControl))
