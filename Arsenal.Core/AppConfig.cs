...
public static class AppConfig
{
    ...
    static AppConfig()
    {
        ...
        string appPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "Arsenal");
        string startupConfig = Path.Combine(Application.StartupPath.Trim('\\'), configName);

        fallbackConfigFile = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "Arsenal", configName);

        // Carry the user's existing configuration into the new product identity once.
        // The split legacy value is intentional: old branding should not leak back into
        // UI, binaries, metadata or future source searches after the migration path is removed.
        string legacyBrand = string.Concat("G", "Helper");
        string legacyUserConfig = Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), legacyBrand, configName);
        string legacyFallbackConfig = Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), legacyBrand, configName);

        Directory.CreateDirectory(appPath);
        CopyLegacyConfig(legacyUserConfig, Path.Combine(appPath, configName));
        CopyLegacyConfig(legacyFallbackConfig, fallbackConfigFile);
    ...
    private static void CopyLegacyConfig(string source, string destination)
    {
        ...
        {
            // A migration failure must never prevent Arsenal from starting with defaults.
        }
    }
    ...
    /// <summary>
    /// Persists pending settings immediately. Normal settings remain debounced; this
    /// is reserved for process hand-offs where a newly elevated instance must observe
    /// the choice before the two-second writer would normally run.
    /// </summary>
    ...
    private static void SyncFallbackConfig()
    {
        ...
        try
        {
            // GetDirectoryName answers null for a path with no directory part. The catch
            // below would have swallowed the resulting exception, so this was never a
            // crash - but it did mean the fallback copy silently stopped happening.
    ...
    {
        string model = GetModel().Trim();
        if (model.Length == 0) return string.Empty;

        // ASUS sometimes reports a marketing name followed by a duplicated SKU,
        // for example "ROG Zephyrus G16 GU605MI_GU605MI". The final token carrying
        // digits is the actual model number; keep that for compact UI labels and
        // model capability checks.
        string[] tokens = model.Replace('_', ' ')
            .Split(' ', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
        for (int i = tokens.Length - 1; i >= 0; i--)
        {
            if (tokens[i].Any(char.IsDigit)) return tokens[i];
        }

        return tokens.Length > 0 ? tokens[^1] : model;
    }

    public static string GetModelDisplayName()
    {
        string model = GetModel().Trim().Replace('_', ' ');
        string modelNumber = GetModelShort();

        // Strip one or more repeated SKU suffixes from the marketing name.
        while (modelNumber.Length > 0
            && model.EndsWith(modelNumber, StringComparison.OrdinalIgnoreCase))
        {
            model = model[..^modelNumber.Length].TrimEnd();
        }

        if (model.Length > 0) return model;

        // Some firmware exposes only the SKU. Keep the homepage useful with the
        // ASUS family name when the marketing prefix is unavailable.
        if (modelNumber.StartsWith("GU605", StringComparison.OrdinalIgnoreCase)) return "ROG Zephyrus G16";
        if (modelNumber.StartsWith("FA507", StringComparison.OrdinalIgnoreCase)) return "TUF Gaming A15";
        if (modelNumber.StartsWith("G614", StringComparison.OrdinalIgnoreCase)) return "ROG Strix G16";
        if (modelNumber.StartsWith("G834", StringComparison.OrdinalIgnoreCase)) return "ROG Strix SCAR 18";
        return "ASUS device";
    }
    ...
    public static int Get(string name, int empty = -1)
    {
        lock (configLock)
        {
            if (!config.TryGetValue(name, out object? value) || value is null) return empty;

            // Values loaded from config.json are JsonElements, while values changed in
            // this process are boxed Int32s. Handle both without allocating a temporary
            // string on every settings read; retain the old string parser as a fallback
            // for any legacy or externally supplied value type.
            if (value is int number) return number;
            if (value is JsonElement element)
            {
                if (element.ValueKind == JsonValueKind.Number && element.TryGetInt32(out number)) return number;
                if (element.ValueKind == JsonValueKind.String && int.TryParse(element.GetString(), out number)) return number;
                return empty;
            }
            if (value is string text && int.TryParse(text, out number)) return number;
            return int.TryParse(value.ToString(), out number) ? number : empty;
        }
    }
