...
namespace Arsenal.Mode
{
    public class ModeControl
    {
        ...
        public static event Action<string>? OnModeLabelChanged;
        public static event Action<string>? OnFansResult;
        public static event Action? OnFansInit;
        public static event Action? OnGPUInit;
        ...
        public void Toast()
        {
            Program.toast.RunToast(Modes.GetCurrentName(), SystemInformation.PowerStatus.PowerLineStatus == PowerLineStatus.Online ? ToastIcon.Charger : ToastIcon.Battery, "Power Profile");
        }
            ...
            if (AppConfig.IsApplyFans() || force)
            {
                ...
                if (cpuResult != 1 || gpuResult != 1)
                {
                    ...
                    if (cpuResult != 1 || gpuResult != 1)
                    {
                        ...
                        OnFansResult?.Invoke("Model doesn't support custom fan curves");
                        Program.Bridge?.LabelFansResult("Model doesn't support custom fan curves");
                    }
                }
                else
                {
                    OnFansResult?.Invoke("");
                    Program.Bridge?.LabelFansResult("");
                    customFans = true;
                }
