...
namespace Arsenal.Input
{

    public class InputDispatcher
    {
        public static event Action? OnToggleApp;
        public static event Action? OnToggleOverlay;
        public static event Action<int>? OnCycleAura;
        public static event Action<int>? OnCycleVisual;
        public static event Action<int>? OnCycleMatrix;
        public static event Action? OnFnLockChanged;
        public static event Action<int>? OnBacklightChanged;
        public static event Action? OnXGMChanged;
        public static event Action<bool>? OnTouchpadChanged;
        /// <summary>ACPI charger connect/disconnect (WMI event 123).</summary>
        public static event Action? OnChargerEvent;
        ...
        public void KeyPressed(object sender, KeyPressedEventArgs e)
        {
            ...
            if (e.Modifier == keyModifier)
            {
                ...
                if (e.Key == keyApp) OnToggleApp?.Invoke();
            ...
            if (e.Modifier == keyModifierAlt)
            {
                ...
                if (e.Key == keyXGM) Program.gpuControl?.ToggleXGM(true);
                if (e.Key == keyOverlay) OnToggleOverlay?.Invoke();
                ...
                switch (e.Key)
                {
                    ...
                    case Keys.F2:
                        ...
                        break;
                    case Keys.F3:
                        Program.gpuControl?.ToggleXGM(true);
                        break;
                    case Keys.F4:
                        Program.allyControl?.ToggleModeHotkey();
                        break;
                    case Keys.F6:
                        ToggleTouchScreen();
                        break;
                    ...
                    case Keys.F8:
                        ...
                        break;
                    case Keys.F9:
                        Program.allyControl?.ToggleFPSLimit(true);
                        break;
                    case Keys.F13:
                        ToggleScreenRate();
                        break;
        ...
        public static void ToggleTouchScreen()
        {
        ...
        /// <summary>
        /// The legacy app uses this Precision TouchPad status value for both capability
        /// and state. A missing value means this machine has no controllable touchpad.
        /// </summary>
        public static bool IsTouchpadAvailable()
        {
            using var key = Registry.CurrentUser.OpenSubKey(
                @"SOFTWARE\Microsoft\Windows\CurrentVersion\PrecisionTouchPad\Status", false);
            return key?.GetValue("Enabled") is not null;
        }
        ...
        public static void ToggleTouchpadEvent(bool hotkey = false)
        {
        ...
        static void HandleOptimizationEvent(int EventID)
        {
            switch (EventID)
            {
                case 16: // FN+F7
                    ...
                    {
                        if (AppConfig.IsDUO()) SetScreenpad(-10);
                        else if (AppConfig.IsAnimeMatrix()) OnCycleMatrix?.Invoke(-1);
                        else SetBacklight(-1);
                    }
        ...
        public static void SetBacklight(int delta, bool force = false)
        {
            ...
            OnBacklightChanged?.Invoke(backlight);
        ...
        static void WatcherEventArrived(object sender, EventArrivedEventArgs e)
        {
            try
            {
                ...
                // A WMI property can come back null, and this runs on the WMI watcher's
                // own thread - an exception here is not caught by anything upstream, so
                // a malformed event would take the process down rather than being
                // ignored. TryParse also covers a value that is present but not a number.
                string? rawEventId = e.NewEvent["EventID"]?.ToString();
                if (!int.TryParse(rawEventId, out int EventID)) return;
                Logger.WriteLine("WMI event " + EventID);
                if (AppConfig.NoWMI()) return;

                if (EventID == 123) OnChargerEvent?.Invoke();
                if (EventID == 186 || EventID == 194) OnXGMChanged?.Invoke();
